...
{   $hotid = $_POST['hotid']; 
	echo "update cjobreg_tb set hot='1' where id='".$hotid."'";
	mysqli_query($con,"update cjobreg_tb set hot='1' where id='".$hotid."'");
    header("location:viewjobc_regcompany.php");
	exit();	
}
else{
if(isset($_POST['hot']))
{   $hotid = $_POST['hotid']; 
    echo "update cjobreg_tb set hot = '0' where id='".$hotid."'";
	mysqli_query($con,"update cjobreg_tb set hot = '0' where id='".$hotid."'");
	if($con){
	mysqli_query($con,"update cjobreg_tb set hot = '0' where id='".$hotid."'");
	//mysqli_query($con,"update cjobreg_tb set `date_time` =  '000-00-00 00:00:00' where id='".$hotid."'");
	}	
    header("location:viewjobc_regcompany.php");
	exit();	
}
...
<?php $page_name="viewjobc_regcompany.php"; //  If you use this code with a different page ( or file ) name then change this 
...
?> 
										<tbody>
											<tr> 
												...
												<td width="100">
													<center>
														<a onclick="return confirm('Are you sure you want to remove this Form ?')" href="viewjobc_regcompany.php?del=<?php echo $id; ?>"> <img src="img/delete.png" width="15"> </a> |	<a style="font-size:10px;padding:3px 3px;background:#1a8646;color:#fff;" href="viewjobc_company_details.php?id=<?php echo $id; ?>">View</a>
													</center>
												</td>
											</tr>
										</tbody>
...
if(isset($_GET['del'])){
    ...
    $d = "delete from cjobreg_tb where id = '$id' ";
	
    $del = mysqli_query($con,$d);
	if($del){		
       header("location:viewjob_regcompany.php?d=1");
	   exit();
	}
}
?>
